...
export default function PopularTransfer({ city_region_id }) {
  ...
  useEffect(() => {
    ...
    const fetchTours = async () => {
      setLoading(true);
      try {
        const relatedData = await getTransfersByCity(
          city_region_id.id ?? city_region_id,
        );

        console.log("API DATA 👉", relatedData);

        let related = relatedData.transfers || [];
  ...
  return (
    <div className="flex-1">
      <h2 className="text-black text-3xl font-bold mb-3">Most popular Tour</h2>
      <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6 gap-y-2 my-4">
        {tours.map((tour) => (
          ...
          >
            <Link href={`/transfer-details/${tour._id}`}>
              ...
              <div className="px-5">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <p className="text-sm text-gray-500">{tour.timing}</p>
                </div>
              </div>
            </Link>
          </div>
